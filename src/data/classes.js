export const DEFAULT_CLASS_ID = "warrior";

export const classDefs = {
  warrior: {
    id: "warrior",
    name: "Krieger",
    role: "Tank / Kontrolle",
    fantasy: "Steht vorne, bindet Gegner und oeffnet sichere Fenster fuer Team-Schaden.",
    color: "#d94b3d",
    accent: "#f4c95d",
    stats: {
      maxHp: 210,
      speed: 215,
      baseAttack: 9,
      attackBonus: 3,
      armorLevel: 3,
      hpPerLevel: 26,
      attackPerLevel: 1.6,
    },
    weaponStyle: "sword",
    starterWeapon: "rust_sword",
    bodyAccent: "horned-helm",
    bars: { leben: 10, schaden: 6, tempo: 4, team: 9 },
    abilities: ["shieldBash", "whirlwind", "earthquake"],
    passive: {
      name: "Eisenhaut",
      text: "Weniger eingehender Schaden, wenn mehrere Gegner nah an dir sind.",
    },
  },
  shadow: {
    id: "shadow",
    name: "Schattenlaeufer",
    role: "Burst / Mobilitaet",
    fantasy: "Springt in Luecken, markiert Ziele und bestraft betaeubte Gegner.",
    color: "#6f63ff",
    accent: "#35d0a4",
    stats: {
      maxHp: 120,
      speed: 320,
      baseAttack: 13,
      attackBonus: 3,
      armorLevel: 0,
      hpPerLevel: 17,
      attackPerLevel: 2.0,
    },
    weaponStyle: "dagger",
    starterWeapon: "twin_daggers",
    bodyAccent: "hood",
    bars: { leben: 4, schaden: 10, tempo: 10, team: 6 },
    abilities: ["shadowStep", "poisonMark", "shadowDouble"],
    passive: {
      name: "Rueckenjaeger",
      text: "Mehr Schaden gegen markierte, betaeubte oder geschwaechte Ziele.",
    },
  },
  druid: {
    id: "druid",
    name: "Hain-Druidin",
    role: "Nature / Crowd Control",
    fantasy: "Verwurzelt Gegner, beschwört Schwärme und verwandelt sich in eine Bärin.",
    color: "#3f7d35",
    accent: "#a3e635",
    stats: {
      maxHp: 165,
      speed: 240,
      baseAttack: 9,
      attackBonus: 3,
      armorLevel: 1,
      hpPerLevel: 22,
      attackPerLevel: 1.7,
    },
    weaponStyle: "staff",
    starterWeapon: "apprentice_staff",
    bodyAccent: "wizard-hat",
    bars: { leben: 6, schaden: 6, tempo: 6, team: 9 },
    abilities: ["rootSnare", "swarmCall", "bearForm"],
    passive: {
      name: "Naturverbunden",
      text: "Regeneriert 3 HP/s in allen Welten, +50% in der Nähe verwurzelter Gegner.",
    },
  },
  charmer: {
    id: "charmer",
    name: "Lyra die Verfuehrerin",
    role: "Charm / Glas-Kanone",
    fantasy: "Tanzt um ihre Polstange, charmt Gegner und dreht das Schlachtfeld um.",
    color: "#ec4899",
    accent: "#f5d042",
    stats: {
      maxHp: 130,
      speed: 290,
      baseAttack: 11,
      attackBonus: 3,
      armorLevel: 0,
      hpPerLevel: 20,
      attackPerLevel: 1.7,
    },
    weaponStyle: "pole",
    starterWeapon: "dancer_pole",
    bodyAccent: "diadem",
    bars: { leben: 4, schaden: 8, tempo: 9, team: 9 },
    abilities: ["poleSpin", "blowKiss", "charmDance"],
    passive: {
      name: "Anmut",
      text: "+20% Ausweich-Chance. Jeder Treffer baut Charme-Stacks (max 5) - bei 5 Stacks kritet der naechste Skill garantiert und heilt 8% maxHP.",
    },
  },
  runemage: {
    id: "runemage",
    name: "Runenmagier",
    role: "AoE / Support",
    fantasy: "Kontrolliert Flaechen, detoniert Marken und schuetzt kurze Team-Momente.",
    color: "#2aa66f",
    accent: "#f2d06b",
    stats: {
      maxHp: 145,
      speed: 250,
      baseAttack: 8,
      attackBonus: 4,
      armorLevel: 1,
      hpPerLevel: 18,
      attackPerLevel: 1.8,
    },
    weaponStyle: "staff",
    starterWeapon: "apprentice_staff",
    bodyAccent: "wizard-hat",
    bars: { leben: 5, schaden: 8, tempo: 5, team: 10 },
    abilities: ["fireOrb", "frostCircle", "meteor"],
    passive: {
      name: "Runenfluss",
      text: "Skills laden etwas schneller, wenn du Gegner auf Abstand haeltst.",
    },
  },
};

export function getClassDef(classId) {
  return classDefs[classId] || classDefs[DEFAULT_CLASS_ID];
}
